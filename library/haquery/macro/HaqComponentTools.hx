...
class HaqComponentTools 
{
	...
	/*
	public static function managers(ethis:Expr)
	{
		var pos = Context.currentPos();
		...
		switch (Context.typeof(ethis))
		{
			case Type.TInst(t, params):
				...
				if (HaqTools.isExtendsFrom(clas, "haquery.server.HaqComponent"))
				{
					return { expr:ExprDef.ENew("haquery.autogenerated.ManagersServer".asTypePath(), [ ethis ]), pos:pos };
				}
				else
				if (HaqTools.isExtendsFrom(clas, "haquery.client.HaqComponent"))
				{
					return { expr:ExprDef.ENew("haquery.autogenerated.ManagersClient".asTypePath(), [ ethis ]), pos:pos };
				}
			default:
		}
