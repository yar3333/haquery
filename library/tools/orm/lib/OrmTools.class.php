...
class OrmTools {
	...
	static function field2var($f) {
		...
		$製pos = $GLOBALS['%s']->length;
		{
			$裨mp = _hx_anonymous(array("name" => $f->name, "type" => $f->type, "isNull" => $f->isNull, "isKey" => $f->isKey, "isAutoInc" => $f->isAutoInc, "haxeName" => $f->name, "haxeType" => OrmTools::sqlType2haxeType($f->type), "haxeDefVal" => (($f->name === "position") ? "null" : null)));
			$GLOBALS['%s']->pop();
			return $裨mp;
		}
		$GLOBALS['%s']->pop();
