...
function TrmHaxeClass_1(&$body, &$isPrivate, &$isStatic, &$name, &$retType, &$vars, $v) {
	$»spos = $GLOBALS['%s']->length;
	{
		...
		{
			$»tmp = $v->name . ":" . $v->type . (TrmHaxeClass_5($»this, $body, $isPrivate, $isStatic, $name, $retType, $v, $vars));
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
...
function TrmHaxeClass_5(&$»this, &$body, &$isPrivate, &$isStatic, &$name, &$retType, &$v, &$vars) {
	$»spos = $GLOBALS['%s']->length;
	if($v->defVal !== null) {
		return "=" . $v->defVal;
	} else {
