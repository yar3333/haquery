...
class haquery_base_HaqTools {
	...
	static function bool($v) {
		$GLOBALS['%s']->push("haquery.base.HaqTools::bool");
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = !_hx_equal($v, false) && $v !== null && !_hx_equal($v, 0) && !_hx_equal($v, "") && !_hx_equal($v, "0") && !_hx_equal($v, "false") && !_hx_equal($v, "off");
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
	...
	static function object2hash($obj) {
		$GLOBALS['%s']->push("haquery.base.HaqTools::object2hash");
		$»spos = $GLOBALS['%s']->length;
		$r = new Hash();
		{
