...
class TrmGenerator
{
	...
	public static function run(manager:HaqTemplateManager, fs:FileSystemTools)
	{
		new TrmGenerator(manager, fs);
	}
	
	function new(manager:HaqTemplateManager, fs:FileSystemTools)
    {
	...
	function generate(fullTag:String)
	{
		...
		{
			fs.deleteFile(template.genBaseServerFilePath);
		}
		...
		{
			fs.deleteFile(template.genBaseClientFilePath);
		}
		...
		if (template.hasLocalServerClass || template.hasLocalClientClass)
		{
			...
			{
				fs.deleteFile(template.genTemplateServerFilePath);
			}
			...
			{
				fs.deleteFile(template.genTemplateClientFilePath);
			}
		}
	}
	...
	function generateTypedefFile(className:String, classFilePath:String, extend:String)
	{
		...
		fs.createDirectory(Path.directory(classFilePath));
