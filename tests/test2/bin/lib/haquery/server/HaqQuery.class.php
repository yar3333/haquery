...
class haquery_server_HaqQuery {
	public function __construct($prefixID, $query, $nodes) {
		...
		$this->query = $query;
		$this->nodes = haquery_server_HaqQuery_0($this, $nodes, $prefixID, $query);
		$GLOBALS['%s']->pop();
	}}
	public $prefixID;
	public $query;
	...
	public function get($index) {
		...
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = haquery_server_HaqQuery_1($this, $index);
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
	...
	public function attr($name, $value) {
		...
		$»spos = $GLOBALS['%s']->length;
		if($value === null) {
			$»tmp = haquery_server_HaqQuery_2($this, $name, $value);
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		{
	...
	public function addClass($clas) {
		...
		$»spos = $GLOBALS['%s']->length;
		$classes = _hx_deref(new EReg("\\s+", ""))->split($clas);
		{
			$_g = 0; $_g1 = $this->nodes;
			while($_g < $_g1->length) {
				$node = $_g1[$_g];
				++$_g;
				$s = haquery_server_HaqQuery_3($this, $_g, $_g1, $clas, $classes, $node);
				{
					$_g2 = 0;
					while($_g2 < $classes->length) {
						$c = $classes[$_g2];
						++$_g2;
						if(!_hx_deref(new EReg("(^|\\s)" . $c . "(\\s|\$)", ""))->match($s)) {
							$s .= " " . $c;
						}
						unset($c);
					}
	...
	public function hasClass($clas) {
		...
		$classes = _hx_deref(new EReg("\\s+", ""))->split($clas);
		{
			...
			while($_g < $_g1->length) {
				$node = $_g1[$_g];
				++$_g;
				$s = haquery_server_HaqQuery_4($this, $_g, $_g1, $clas, $classes, $node);
				$inAll = true;
				{
					$_g2 = 0;
					while($_g2 < $classes->length) {
	...
	public function removeClass($clas) {
		...
		$classes = _hx_deref(new EReg("\\s+", ""))->split($clas);
		{
			...
			while($_g < $_g1->length) {
				$node = $_g1[$_g];
				++$_g;
				$s = haquery_server_HaqQuery_5($this, $_g, $_g1, $clas, $classes, $node);
				{
					$_g2 = 0;
					while($_g2 < $classes->length) {
						$c = $classes[$_g2];
						++$_g2;
						$s = _hx_deref(new EReg("(^|\\s)" . $c . "(\\s|\$)", ""))->replace($s, " ");
						unset($c);
					}
					unset($_g2);
				}
	...
	public function html($html, $isParse) {
		...
		if($html === null) {
			$»tmp = haquery_server_HaqQuery_6($this, $html, $isParse);
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		{
	...
	public function css($name, $val) {
		...
		$»spos = $GLOBALS['%s']->length;
		if(call_user_func_array($__phyeq, array($val, null))) {
			if($this->nodes->length > 0) {
				$sStyles = _hx_array_get($this->nodes, 0)->getAttribute("style");
				$re = new EReg("\\b(" . $name . ")\\b\\s*:\\s*(.*?)\\s*;", "");
				if($re->match($sStyles)) {
					$»tmp = $re->matched(1);
					$GLOBALS['%s']->pop();
					return $»tmp;
		...
		$re = new EReg("\\b(" . $name . ")\\b\\s*:\\s*(.*?)\\s*(;|\$)", "");
		{
			...
			while($_g < $_g1->length) {
				...
				$sStyles = _hx_array_get($this->nodes, 0)->getAttribute("style");
				if($re->match($sStyles)) {
					$sStyles = $re->replace(haquery_server_HaqQuery_7($this, $_g, $_g1, $name, $node, $re, $sStyles, $val), $sStyles);
				} else {
					if(!$val === "" && !$val === null) {
...
function haquery_server_HaqQuery_0(&$»this, &$nodes, &$prefixID, &$query) {
	if($nodes !== null) {
		return new _hx_array($nodes);
	} else {
		return new _hx_array(array());
	}
}
function haquery_server_HaqQuery_1(&$»this, &$index) {
	if($index === null) {
		return $»this->nodes;
	} else {
...
function haquery_server_HaqQuery_2(&$»this, &$name, &$value) {
	if($»this->nodes->length > 0) {
		return _hx_array_get($»this->nodes, 0)->getAttribute($name);
	}
}
function haquery_server_HaqQuery_3(&$»this, &$_g, &$_g1, &$clas, &$classes, &$node) {
...
function haquery_server_HaqQuery_4(&$»this, &$_g, &$_g1, &$clas, &$classes, &$node) {
	if($node->hasAttribute("class")) {
		return $node->getAttribute("class");
	} else {
		return "";
...
function haquery_server_HaqQuery_5(&$»this, &$_g, &$_g1, &$clas, &$classes, &$node) {
	if($node->hasAttribute("class")) {
		return $node->getAttribute("class");
	} else {
		return "";
...
function haquery_server_HaqQuery_6(&$»this, &$html, &$isParse) {
	if($»this->nodes->length > 0) {
		return _hx_array_get($»this->nodes, 0)->innerHTML;
	}
}
function haquery_server_HaqQuery_7(&$»this, &$_g, &$_g1, &$name, &$node, &$re, &$sStyles, &$val) {
	if(!$val === "" && !$val === null) {
		return $name . ": " . $val . ";";
	} else {
		return "";
