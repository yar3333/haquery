...
class haquery_server_HaqComponent extends haquery_base_HaqComponent {
	...
	public function createChildComponents_inner($baseNode) {
		...
		while($i < count($baseNode->children)) {
			...
			haquery_base_HaQuery::assert($node->name !== "haq:placeholder", null, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 95, "className" => "haquery.server.HaqComponent", "methodName" => "createChildComponents_inner")));
			haquery_base_HaQuery::assert($node->name !== "haq:content", null, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 96, "className" => "haquery.server.HaqComponent", "methodName" => "createChildComponents_inner")));
	...
	public function render() {
		...
		if(haquery_base_HaQuery::$config->isTraceComponent) {
			haxe_Log::trace("render " . $this->fullID, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 158, "className" => "haquery.server.HaqComponent", "methodName" => "render")));
		}
...
function haquery_server_HaqComponent_0(&$»this, &$method, &$params) {
	if(strlen($»this->fullID) !== 0) {
		return "haquery.client.HaQuery.page.findComponent('" . $»this->fullID . "')." . $method;
	} else {
		return "haquery.client.HaQuery.page." . $method;
	}
}
