...
class haquery_server_HaqComponent extends haquery_base_HaqComponent {
	...
	public function construct($manager, $parent, $tag, $id, $doc, $params, $innerHTML) {
		...
		$»spos = $GLOBALS['%s']->length;
		parent::commonConstruct($manager,$parent,$tag,$id);
		$this->doc = $doc;
		$this->params = $params;
		...
		if(Reflect::hasMethod($this, "init")) {
	...
	public function loadParamsToObjectFields() {
		...
		if($this->params !== null) {
			...
			$fields = new Hash();
			{
				$_g = 0; $_g1 = Reflect::publicVars($this);
				while($_g < $_g1->length) {
					$field = $_g1[$_g];
					++$_g;
					if($restrictedFields->indexOf($field) === -1 && !_hx_starts_with($field, "event_")) {
	...
	public function createChildComponents_inner($baseNode) {
		...
		while($i < count($baseNode->children)) {
			...
			haquery_base_HaQuery::assert($node->name !== "haq:placeholder", null, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 89, "className" => "haquery.server.HaqComponent", "methodName" => "createChildComponents_inner")));
			haquery_base_HaQuery::assert($node->name !== "haq:content", null, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 90, "className" => "haquery.server.HaqComponent", "methodName" => "createChildComponents_inner")));
			if(_hx_starts_with($node->name, "haq:")) {
	...
	public function render() {
		...
		if(haquery_base_HaQuery::$config->isTraceComponent) {
			haxe_Log::trace("render " . $this->fullID, _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 157, "className" => "haquery.server.HaqComponent", "methodName" => "render")));
		}
	...
	public function q($query) {
		...
		if(Type::getClassName(Type::getClass($query)) !== "String") {
			haquery_base_HaQuery::error("HaqComponent.q() error - 'query' parameter must be a string or HaqQuery!", _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 175, "className" => "haquery.server.HaqComponent", "methodName" => "q")));
		}
	...
	public function callJsMethod($jsCode) {
		$GLOBALS['%s']->push("haquery.server.HaqComponent::callJsMethod");
		$»spos = $GLOBALS['%s']->length;
		haquery_server_HaqInternals::addAjaxAnswer(haquery_server_HaqComponent_0($this, $jsCode));
		$GLOBALS['%s']->pop();
	}
...
function haquery_server_HaqComponent_0(&$»this, &$jsCode) {
	if(strlen($»this->fullID) !== 0) {
		return "haquery.client.HaQuery.page.findComponent('" . $»this->fullID . "')." . $jsCode . ";";
	} else {
		return "haquery.client.HaQuery.page." . $jsCode . ";";
	}
}
