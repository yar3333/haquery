...
class haquery_server_HaqComponentManager {
	...
	static function getNameByTag($tag) {
		...
		if(!_hx_starts_with($tag, "haq:")) {
			haquery_base_HaQuery::error("Component tag \"" . $tag . "\" must started with \"haq:\".", _hx_anonymous(array("fileName" => "HaqComponentManager.hx", "lineNumber" => 78, "className" => "haquery.server.HaqComponentManager", "methodName" => "getNameByTag")));
		}
		{
...
function haquery_server_HaqComponentManager_1(&$components, &$s, &$tag, &$tags, $x) {
	{
		$GLOBALS['%s']->push("haquery.server.HaqComponentManager::getInternalDataForPageHtml@68");
