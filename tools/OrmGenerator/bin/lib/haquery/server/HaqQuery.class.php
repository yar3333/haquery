	...
	public function css($name, $val) {
		...
		$re = new EReg("\\b(" . $name . ")\\b\\s*:\\s*(.*?)\\s*(;|\$)", "");
		{
			...
			while($_g < $_g1->length) {
				...
				$sStyles = _hx_array_get($this->nodes, 0)->getAttribute("style");
				if($re->match($sStyles)) {
					$sStyles = $re->replace(haquery_server_HaqQuery_6($this, $_g, $_g1, $name, $node, $re, $sStyles, $val), $sStyles);
				} else {
					if(!$val === "" && !$val === null) {
...
function haquery_server_HaqQuery_6(&$»this, &$_g, &$_g1, &$name, &$node, &$re, &$sStyles, &$val) {
	if(!$val === "" && !$val === null) {
		return $name . ": " . $val . ";";
	} else {
		return "";
