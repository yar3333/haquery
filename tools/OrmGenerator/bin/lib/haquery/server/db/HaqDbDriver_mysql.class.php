...
class haquery_server_db_HaqDbDriver_mysql implements haquery_server_db_HaqDbDriver{
	...
	public function query($sql) {
		...
		$»spos = $GLOBALS['%s']->length;
		if(haquery_base_HaQuery::$config->sqlTraceLevel >= 2) {
		...
		$errno = mysql_errno();
		if($errno !== 0 && haquery_base_HaQuery::$config->stopOnSqlError) {
			haquery_base_HaQuery::error("sql query error:" . "\x0ASQL QUERY: " . $sql . "\x0ASQL RESULT: " . $this->affectedRows() . " rows affected." . " Error code = " . $errno . ": " . $this->getLastErrorMessage() . ".", _hx_anonymous(array("fileName" => "HaqDbDriver_mysql.hx", "lineNumber" => 32, "className" => "haquery.server.db.HaqDbDriver_mysql", "methodName" => "query")));
		}
		if(haquery_base_HaQuery::$config->sqlTraceLevel > 0) {
			if(haquery_base_HaQuery::$config->sqlTraceLevel === 1 && $errno !== 0) {
				haxe_Log::trace("SQL QUERY: " . $sql, _hx_anonymous(array("fileName" => "HaqDbDriver_mysql.hx", "lineNumber" => 41, "className" => "haquery.server.db.HaqDbDriver_mysql", "methodName" => "query")));
			}
			if(haquery_base_HaQuery::$config->sqlTraceLevel >= 3 || $errno !== 0) {
				haxe_Log::trace("SQL RESULT: " . $this->affectedRows() . " rows affected. Error code = " . $errno . ": " . $this->getLastErrorMessage() . ".", _hx_anonymous(array("fileName" => "HaqDbDriver_mysql.hx", "lineNumber" => 43, "className" => "haquery.server.db.HaqDbDriver_mysql", "methodName" => "query")));
			}
		}
		{
			$GLOBALS['%s']->pop();
	...
	public function quote($v) {
		...
		haquery_base_HaQuery::error("Unsupported parameter type '" . Type::getClassName(Type::getClass($v)) . "'.", _hx_anonymous(array("fileName" => "HaqDbDriver_mysql.hx", "lineNumber" => 108, "className" => "haquery.server.db.HaqDbDriver_mysql", "methodName" => "quote")));
		{
			$GLOBALS['%s']->pop();
			return null;
		}
