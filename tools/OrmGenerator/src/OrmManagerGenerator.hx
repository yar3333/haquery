...
class OrmManagerGenerator 
{
	...
	static function getAutoGeneratedManager(table:String, vars:List<OrmHaxeVar>, modelFullClassName:String, fullClassName:String, baseFullClassName:String=null) : OrmHaxeClassGenerator
	{
		var model:OrmHaxeClassGenerator = new OrmHaxeClassGenerator(fullClassName, baseFullClassName);
		
		model.addImport('php.db.ResultSet');
		model.addImport('haquery.server.db.HaqDb');
		
		model.addMethod('modelByParams', vars, modelFullClassName,
		...
		model.addMethod('modelByRow', [ OrmTools.createVar('d', 'Dynamic') ], modelFullClassName,
			 "var _obj = new " + modelFullClassName + "();\n"
		...
		if (getVars.length > 0)
		{
			model.addMethod('get', getVars, modelFullClassName,
				 "return object('SELECT * FROM `" + table + "` WHERE " 
					+Lambda.map(getVars, function(v:OrmHaxeVar) { return "`" + v.name + "` = ' + HaqDb.quote(" + v.haxeName + ")"; } ).join("+' AND ")
				+");"
			);
		}
		...
		model.addMethod('create', createVars, modelFullClassName,
			...
			+"return modelByParams(" + Lambda.map(vars, function(v) { return v.isAutoInc ? 'HaqDb.lastInsertId()' : v.haxeName; } ).join(", ") + ");"
		);
		...
		model.addMethod('delete', deleteVars, 'Void',
			 "HaqDb.query('DELETE FROM `" + table + "` WHERE " 
				+Lambda.map(deleteVars, function(v:OrmHaxeVar) { return "`" + v.name + "` = ' + HaqDb.quote(" + v.haxeName + ")"; } ).join("+' AND ")
			+" + ' LIMIT 1');"
		);
		
		model.addMethod('all', [ OrmTools.createVar('_order', 'String', getOrderDefVal(vars)) ], 'Array<'+modelFullClassName+'>',
			 "return objects('SELECT * FROM `" + table + "`' + (_order != null ? ' ORDER BY ' + _order : ''));"
		);
		
		model.addMethod('object', [ OrmTools.createVar('sql', 'String') ], modelFullClassName,
			 "var rows : ResultSet = HaqDb.query(sql + ' LIMIT 1');\n"
			+"if (rows.length == 0) return null;\n"
			+"return modelByRow(rows.next());"
		);
		
		model.addMethod('objects', [ OrmTools.createVar('sql', 'String') ], 'Array<'+modelFullClassName+'>',
		...
		for (fk in HaqDb.connection.getForeignKeys(table))
		{
			var vs = Lambda.filter(vars, function(v) { return v.name == fk.key; } );
			createGetsByMethod(table, vars, modelFullClassName, vs, model);
		}
		
		return model;
	}
	
	static function getCustomManager(table:String, vars:List<OrmHaxeVar>, modelFullClassName:String, fullClassName:String, baseFullClassName:String=null) : OrmHaxeClassGenerator
	{
		var model:OrmHaxeClassGenerator = new OrmHaxeClassGenerator(fullClassName, baseFullClassName);
	...
	static function createGetByMethod(table:String, vars:List<OrmHaxeVar>, modelFullClassName:String, whereVars:List<OrmHaxeVar>, model:OrmHaxeClassGenerator) : Void
	{
		model.addMethod(
	...
	static function createGetsByMethod(table:String, vars:List<OrmHaxeVar>, modelFullClassName:String, whereVars:List<OrmHaxeVar>, model:OrmHaxeClassGenerator) : Void
	{
		model.addMethod(
			'getBy' + Lambda.map(whereVars, function(v) { return OrmTools.capitalize(v.haxeName); } ).join('And'),
			...
			'Array<' + modelFullClassName + '>',
			 ...
			 "return objects('SELECT * FROM `" + table + "` WHERE " 
				+Lambda.map(whereVars, function(v:OrmHaxeVar) { return "`" + v.name + "` = ' + HaqDb.quote(" + v.haxeName + ")"; } ).join("+' AND ")
			+" + (_order != null ? ' ORDER BY ' + _order : ''));"
		);
	}
