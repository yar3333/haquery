...
class haquery_server_HaqTools {
	...
	static function getCallClientFunctionString($func, $params) {
		...
		$»spos = $GLOBALS['%s']->length;
		{
			$»tmp = $func . "(" . ((($params !== null) ? Lambda::map($params, (isset(haquery_server_HaqTools::$serverVarToClientString) ? haquery_server_HaqTools::$serverVarToClientString: array("haquery_server_HaqTools", "serverVarToClientString")))->join(", ") : "")) . ")";
			$GLOBALS['%s']->pop();
			return $»tmp;
		}
		$GLOBALS['%s']->pop();
