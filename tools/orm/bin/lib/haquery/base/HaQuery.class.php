...
class haquery_base_HaQuery {
	...
	static function path2url($path) {
		...
		$製pos = $GLOBALS['%s']->length;
		$realPath = str_replace("\\", "/", php_FileSystem::fullPath("")) . "/" . trim($path, "/\\");
	...
	static function trace($v, $pos) {
		...
		if($text !== "") {
			...
			if(!$isHeadersSent) {
				try {
					if(_hx_starts_with($text, "HAXE EXCEPTION")) {
						FirePHP::getInstance(true)->error($text, null, null);
					} else {
						if(_hx_starts_with($text, "HAQUERY")) {
	...
	static function traceException($e) {
		...
		haquery_base_HaQuery::assert($nativeStack !== null, null, _hx_anonymous(array("fileName" => "HaQuery.hx", "lineNumber" => 237, "className" => "haquery.base.HaQuery", "methodName" => "traceException")));
		$text .= "\x0A\x0A";
		...
		haxe_Log::trace($text, _hx_anonymous(array("fileName" => "HaQuery.hx", "lineNumber" => 254, "className" => "haquery.base.HaQuery", "methodName" => "traceException")));
		$GLOBALS['%s']->pop();
	}
...
function haquery_base_HaQuery_0(&$f, &$pos, &$text, &$v) {
	if($text !== "") {
		return sprintf("%.3f", Date::now()->getTime() - haquery_base_HaQuery::$startTime) . " " . str_replace("\x0A", "\x0A\x09", $text) . "\x0A";
	} else {
		return "\x0A";
	}
}
