...
class haquery_base_HaqComponent {
	...
	public function commonConstruct($parent, $tag, $id) {
		...
		$»spos = $GLOBALS['%s']->length;
		if($id === null || $id === "") {
			$id = haquery_base_HaqComponent_0($this, $id, $parent, $tag);
		}
		$this->parent = $parent;
		$this->tag = $tag;
		$this->id = $id;
		$this->fullID = (haquery_base_HaqComponent_1($this, $id, $parent, $tag)) . $id;
		$this->prefixID = haquery_base_HaqComponent_2($this, $id, $parent, $tag);
		if($parent !== null) {
			haquery_base_HaQuery::assert(!$parent->components->exists($id), "Component with same id '" . $id . "' already exist.", _hx_anonymous(array("fileName" => "HaqComponent.hx", "lineNumber" => 71, "className" => "haquery.base.HaqComponent", "methodName" => "commonConstruct")));
...
function haquery_base_HaqComponent_2(&$»this, &$id, &$parent, &$tag) {
	if($»this->fullID !== "") {
		return $»this->fullID . "-";
	} else {
		return "";
