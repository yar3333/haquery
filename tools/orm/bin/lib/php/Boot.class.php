...
function _hx_string_call($s, $method, $params) {
	...
	switch($method) {
		...
		case 'replace'    : return str_replace($params[0], $params[1], $s);
		case 'ltrim'      : return ltrim($s, (count($params) > 0 ? $params[0] : null));
		case 'rtrim'      : return rtrim($s, (count($params) > 0 ? $params[0] : null));
		case 'trim'       : return trim($s, (count($params) > 0 ? $params[0] : null));
		case 'startsWith' : return _hx_starts_with($s, $params[0]);
		case 'endsWith'   : return _hx_ends_with($s, $params[0]);
...
function _hx_starts_with($s, $t)
{
	return strlen($s) >= strlen($t) ? substr($s,0,strlen($t))===$t : false;
}

function _hx_ends_with($s, $t)
{
	return strlen($s) >= strlen($t) ? substr($s, strlen($s)-strlen($t))===$t : false;
}
...
class HException extends Exception {
	public function __construct($e, $message = null, $code = null, $p = null) {
		...
		$this->e = $e;
		$this->p = $p;
		$GLOBALS['%nativeExceptionCallStack'] = debug_backtrace();
	}
	public $e;
	public $p;
