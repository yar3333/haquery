...
class php_Web {
	...
	static function UPLOAD_ERR_OK_getter() {
		$GLOBALS['%s']->push("php.Web::UPLOAD_ERR_OK_getter");
		$製pos = $GLOBALS['%s']->length;
		{
			$裨mp = UPLOAD_ERR_OK;
			$GLOBALS['%s']->pop();
	...
	static function getFiles() {
		...
		while($蜴t->hasNext()) {
			$id = $蜴t->next();
			$file = $files->get($id);
			$r->set($id, _hx_anonymous(array("name" => $file["name"], "type" => $file["type"], "tmp_name" => $file["tmp_name"], "error" => $file["error"], "size" => $file["size"])));
			unset($file);
		}
		{
			$GLOBALS['%s']->pop();
