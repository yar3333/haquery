...
class OrmGenerator
{
    public static function run(db:HaqDb, log:Log, basePath:String)
	{
		new OrmGenerator(db, log, basePath);
	}
	
	function new(db:HaqDb, log:Log, basePath:String)
    {
        ...
        var tables = db.connection.getTables();
        for (table in tables)
        {
			var className = getClassName(table);
			OrmModelGenerator.make(db, log, table, basePath, 'models.' + className, 'models.autogenerated.' + className, 'managers.' + className + 'Manager');
			OrmManagerGenerator.make(db, log, table, basePath, 'models.' + className, 'managers.' + className + 'Manager', 'managers.autogenerated.' + className + 'Manager');
        }
    }
