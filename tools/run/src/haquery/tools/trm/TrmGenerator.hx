...
class TrmGenerator
{
	...
	function generate(fullTag:String)
	{
		...
		{
			generateTypedefFile(fullTag + ".Server", template.stubeServerFilePath, template.extend, "haquery.server.HaqComponent", template.lastMod);
			hant.deleteFile(template.baseServerFilePath);
		}
		else
		{
			hant.deleteFile(template.stubeServerFilePath);
			generateTypedefFile(fullTag + ".BaseServer", template.baseServerFilePath, template.extend, "haquery.server.HaqComponent", template.lastMod);
		}
		
		if (!template.hasLocalClientClass)
		{
			generateTypedefFile(fullTag + ".Client", template.stubeClientFilePath, template.extend, "haquery.client.HaqComponent", template.lastMod);
			hant.deleteFile(template.baseClientFilePath);
		}
		else
		{
			hant.deleteFile(template.stubeClientFilePath);
			generateTypedefFile(fullTag + ".BaseClient", template.baseClientFilePath, template.extend, "haquery.client.HaqComponent", template.lastMod);
		}
	...
	function generateTypedefFile(className:String, classFilePath:String, extend:String, stdSuperClass:String, lastMod:Date)
	{
		if (!FileSystem.exists(classFilePath) || FileSystem.stat(classFilePath).mtime.getTime() < lastMod.getTime())
		{
			...
			File.saveContent(classFilePath, 
				...
				+ "package " + pack + ";\n\n"
				+ "typedef " + name + " = " + (extend != null && extend != "" ? extend + "." + name : stdSuperClass + ";\n")
			);
		}
	}
